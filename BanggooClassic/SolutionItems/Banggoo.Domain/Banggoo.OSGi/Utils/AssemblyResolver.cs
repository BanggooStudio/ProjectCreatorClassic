...
namespace Banggoo.OSGi.Utils
{
    ...
    internal class AssemblyResolver : MarshalByRefObject
    {
        ...
        public void Init(Byte[] assemblyByteArray, string bundleLibPath, string shareLibPath)
        {
            ...
            {
/*对于程序集特性中如存在其他程序集定义的特性，则简化处理，无法读取厂商信息内容*/
            }
            AppDomain.CurrentDomain.ReflectionOnlyAssemblyResolve -= CurrentDomainOnReflectionOnlyAssemblyResolve;
        }
        ...
        private Assembly CurrentDomainOnReflectionOnlyAssemblyResolve(object sender, ResolveEventArgs args)
        {
            try
            {
                ...
                {
                    string[] assemblyFiles = Directory.GetFiles(this.shareLib,
                        string.Format("{0}.dll", resovleAssemblyName.Name), SearchOption.AllDirectories);
                ...
                {
                    string[] bundleLibFles = Directory.GetFiles(this.bundleLib,
                        string.Format("{0}.dll", resovleAssemblyName.Name), SearchOption.AllDirectories);
                ...
                {
                    string[] rootFiles = Directory.GetFiles(Environment.CurrentDirectory,
                        string.Format("{0}.dll", resovleAssemblyName.Name), SearchOption.AllDirectories);
        ...
        public string GetAssemblyRequiredAssembly()
        {
            ...
            foreach (AssemblyName assemblyName in assembly.GetReferencedAssemblies())
            {
                ...
                sb.Append(string.Format("{0};{1}=\"{2}\"", referenceAssemblyName,
                    BUNDLE_MANIFEST_REQUIRED_BUNDLE_VERSION, assemblyName.Version));
                sb.Append(",");
            }
            if (sb.Length == 0)
            {
