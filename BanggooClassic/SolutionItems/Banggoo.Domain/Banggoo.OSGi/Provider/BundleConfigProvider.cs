...
namespace Banggoo.OSGi.Provider
{
    /// <summary>
    ///     配置信息提供程序
    /// </summary>
    internal static class BundleConfigProvider
    {
        ...
        ///     对于依赖库或共享程序集是否全类型加载
        /// </summary>
        public static bool OSGi_NET_ALLTYPES_LOAD;

        /// <summary>
        ///     是否独立运行
        /// </summary>
        public static bool OSGi_NET_SINGLERUNNING;

        /// <summary>
        ///     是否是Debug模式，差异主要在加载DLL方式上，Debug模式只是通过反射加载，不能做到
        /// </summary>
        public static bool OSGi_NET_IS_DEBUG_MODE;
        ...
        private static void LoadFrameworkConfig()
        {
            ...
            //var sth = ConfigurationManager.AppSettings;
            //Note : .config文件的位置须位于当前执行目录中
        ...
        internal static string GetBundlesDirectory()
        {
            ...
            {
                bundlesDirectoryPath = Path.Combine(Environment.CurrentDirectory,
                    FrameworkConfiguration[OSGi_NET_BUNDLE_STORAGE]);
            }
        ...
        internal static string GetShareLibsDirectory()
        {
            ...
            if (FrameworkConfiguration.ContainsKey(OSGi_NET_SHARELIB_STORAGE))
                shareLibsDirectoryPath = Path.Combine(Environment.CurrentDirectory,
                    FrameworkConfiguration[OSGi_NET_SHARELIB_STORAGE]);
        ...
        internal static string GetBundlesUninstallDirectory()
        {
            ...
            if (FrameworkConfiguration.ContainsKey(OSGi_NET_UNINSTALL_STORAGE))
                bundlesUninstallPath = Path.Combine(Environment.CurrentDirectory,
                    FrameworkConfiguration[OSGi_NET_UNINSTALL_STORAGE]);
